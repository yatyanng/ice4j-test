...
public class IceClient {
  ...
  public SocketAddress getRemotePeerSocketAddress() {
    ...
    log.info("Remote candinate transport address:" + remoteCandidate.getTransportAddress());
    log.info("Remote candinate host address:" + remoteCandidate.getHostAddress());
    log.info("Remote candinate mapped address:" + remoteCandidate.getMappedAddress());
    log.info("Remote candinate relayed address:" + remoteCandidate.getRelayedAddress());
    log.info("Remote candinate reflexive address:" + remoteCandidate.getReflexiveAddress());
    return remoteCandidate.getTransportAddress();
  }

  /**
   * Reads an SDP description from the standard input.In production environment that we can exchange
   * SDP with peer through signaling server(SIP server)
   */
  ...
  private Agent createAgent(int rtpPort, String streamName, boolean isTrickling) throws Throwable {
  ...
  private IceMediaStream createStream(int rtpPort, String streamName, Agent agent)
      throws Throwable {
    long startTime = System.currentTimeMillis();
  ...
  public static final class IceProcessingListener implements PropertyChangeListener {
    ...
    public void propertyChange(PropertyChangeEvent event) {
      ...
      if (state == IceProcessingState.COMPLETED) {
        ...
        log.info("Total ICE processing time: " + (processingEndTime - startTime) + "ms");
        ...
        for (IceMediaStream stream : streams) {
          ...
          for (Component c : components) {
            log.info("------------------------------------------");
            log.info(
                "Component of stream:" + c.getName() + ",selected of pair:" + c.getSelectedPair());
            log.info("------------------------------------------");
          }
        }
