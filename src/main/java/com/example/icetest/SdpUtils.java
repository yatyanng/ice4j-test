...
/**
 * Utilities for manipulating SDP. Some of the utilities in this method <b>do not</b> try to act
 * smart and make a lot of assumptions (e.g. at least one media stream with at least one component)
 * that may not always be true in real life and lead to exceptions. Therefore, make sure you reread
 * the code if reusing it in an application. It should be fine for the purposes of our ice4j
 * examples though.
 *
 * @author Emil Ivov
 */
public class SdpUtils {
  /**
   * Creates a session description containing the streams from the specified <tt>agent</tt> using
   * dummy codecs. This method is unlikely to be of use to integrating applications as they would
   * likely just want to feed a {@link MediaDescription} and have it populated with all the
  ...
  /**
   * Configures <tt>localAgent</tt> the the remote peer streams, components, and candidates
   * specified in <tt>sdp</tt>
   *
   ...
   * @param stream the {@link IceMediaStream} that the candidate is supposed to belong to.
   *
   * @return a newly created {@link RemoteCandidate} matching the content of the specified
   *         <tt>attribute</tt> or <tt>null</tt> if the candidate belonged to a component we don't
   *         have.
   */
  private static RemoteCandidate parseCandidate(Attribute attribute, IceMediaStream stream) {
    ...
    RemoteCandidate cand =
        new RemoteCandidate(transAddr, component, type, foundation, priority, relatedCandidate);
